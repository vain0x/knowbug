...
#### 2015/11/15
* ユーザ定義型の文字列化処理を行う Dll に、VswMethods を能動的に渡すようにした。
* 組み込み型の文字列化処理も ini で指定できるようにした。

#### 2015/11/08
* フォントを「IPAGothic」(10pt)に変更した。
* 変数情報のメモリサイズとバッファサイズの表示書式を微妙に変更した。
* Fullwidth Colon `：` ではなく、ASCII文字のコロン `:` を使うようにした。
* "hsp_64.as" が include されていても API 関数の宣言が64ビット版にならないバグを修正した。
  * ``__hsp64__`` ではなく ``__hsp64__@`` が定義済みかどうかをチェックする。
* リリース (ver: 1.21)
* インスタンスの文字列化は1回しか行わないようにした。
...
#### 2009 12/06 (Sun)
* 開発開始。
* EditControl のフォントを「MS UI Gothic」から「ＭＳ ゴシック」(9pt)に変更した。
