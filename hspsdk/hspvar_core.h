...
typedef struct
{
	...
	TypeRedefined(PDAT *, char *) pt;		// ptr to array
...
typedef struct
{
	...
	//		システム参照・型変換用
	//
	TypeRedefined(PDAT *, void *) (*Cnv)(TypeRedefined(const PDAT *, const void *) buffer, int flag);
	TypeRedefined(PDAT *, void *) (*CnvCustom)(TypeRedefined(const PDAT *, const void *) buffer, int flag);
	PDAT *(*GetPtr)( PVal *pval );

	TypeRedefined(PDAT *, void *) (*ArrayObjectRead)(PVal *pval, int *mptype);// 配列要素の指定 (連想配列/読み出し)
	void (*ArrayObject)( PVal *pval );							// 配列要素の指定 (連想配列/書き込み準備)
	void (*ObjectWrite)(PVal *pval, TypeRedefined(const PDAT *, const void *) data, int type);		// HSPVAR_SUPPORT_NOCONVERT指定時の代入
	...
	//		代入用関数(型の一致が保障されます)
	//
	void (*Set)( PVal *pval, PDAT *pdat, TypeRedefined(const PDAT *, const void *) in );
