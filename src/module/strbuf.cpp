...
static void BlockPtrPrepare(void)
{
	...
	if ( sb == NULL ) throw RunErr_MemOut;
	STRBUF* p = sb;
...
{
	return ptr_cast<STRINF*>( ptr_cast<char*>(ptr) - sizeof(STRINF) );
...
void sbFree( void* ptr )
{
	...
	char*   p   = ptr_cast<char*>( ptr );
	STRBUF* st  = ptr_cast<STRBUF*>( p - sizeof(STRINF) );
	STRINF* inf = GET_INTINF(st);
...
{
	STRBUF* st = ptr_cast<STRBUF*>( ptr_cast<char*>(ptr) - sizeof(STRINF) );
...
// @prm size  : 複写する長さ。
//------------------------------------------------
void sbCopy( char** pptr, const void* data, size_t size )
{
	char*  ptr = *pptr;
	STRBUF* st = ptr_cast<STRBUF*>( ptr - sizeof(STRINF) );
...
//------------------------------------------------
void sbCat( char** pptr, const void* data, size_t size, int mode )
{
	char*  ptr = *pptr;
	STRBUF* st = ptr_cast<STRBUF*>( ptr - sizeof(STRINF) );
	char*    p = st->inf.ptr;
