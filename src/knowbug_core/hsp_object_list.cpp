...
static auto with_count(HspObjectListDelta delta, std::size_t count) -> HspObjectListDelta {
	return new_remove(delta.object_id, delta.index, count);
}
...
auto HspObjectListDelta::indented_name() const -> std::u8string {
	static constexpr auto SPACES = u8"                ";

	auto result = std::u8string{};
	result.reserve(depth * 2 + name.length());
	result += as_utf8(SPACES).substr(0, depth * 2);
	result += name;
	return result;
}
	...
	auto push_remove = [&](std::size_t object_id, std::size_t index) {
		if (!diff.empty()) {
			...
			if (last.kind == HspObjectListDeltaKind::Remove && last.index == index) {
				last = with_count(last, last.count + 1);
				return;
			}
		}

		diff.push_back(new_remove(object_id, index, 1));
		};
...

	{
		...
		while (si < source.size() || ti < target.size()) {
			...
			if (si == source.size() || (ti < target.size() && !target_done[ti])) {
				diff.push_back(new_insert(ti, target[ti]));
				ti++;
				continue;
			}
			...
			if (source[si].object_id == target[ti].object_id) {
				...
				if (!s.equals(t)) {
					diff.push_back(new_update(ti, target[ti]));
				}
...
public:
	...
	auto update(HspObjects& objects) -> std::vector<HspObjectListDelta> override {
	...
	void toggle_expand(std::size_t object_id) override {
