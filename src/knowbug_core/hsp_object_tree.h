...
public:
	...
	virtual auto current_node_id() const->std::size_t = 0;
	...
	// 生存しているなら、子ノードのリストを更新する。そして、フォーカスされたノードのIDに current_node_d を変更する。
	virtual void focus(std::size_t node_id, HspObjectTreeObserver& observer) = 0;
