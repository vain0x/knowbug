...
* 「HSPのフォルダ」とは、 HSPがインストールされているフォルダで、通常は「C:/Program Files/hsp34」となります。何にせよ「hsp3debug.dll」のあるフォルダです。
...
「アドレス」「サイズ」およびメモリダンプは、表示しないように設定することもできます。knowbug.ini をご参照ください。
...
```
	...
	要素名:
		...
		要素リスト
```
* 記号「&」は参照、すなわち何かを指し示すことを表していると思ってください。
...
```hsp
#module
#deffunc mystmt str, int
#defcfunc myfunc int
#global

	mystmt "hello", myfunc(0)
```
...
### 「スクリプト」ノード
実行中のスクリプトファイルがみつかった場合は、その内容が表示されます。
