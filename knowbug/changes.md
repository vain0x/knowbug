...
* 「カスタムノード」機能を作ろうかと思ったがやめた。
  * branch: customNode に残骸がある。
  * スクリプト側からAPIを通してツリービューにノードを追加する。それに対応する文字列が必要になったときに、対応するラベルを呼び出して vsw に書き込みを行わせる、というもの。
  * return の直後で再停止させるのが難しい。
